...
const ImovelCard = ({ imovel, onViewDetails, onWhatsApp }) => {
  ...
  console.log('Dados do imóvel:', imovel.titulo);
  console.log('Estrutura imagens:', imovel.imagens);
  console.log('Debug Imóvel:', imovel.titulo, 'Tem imagem:', !!imovel.imagens?.length);
  
  if (imovel.imagens?.data?.length > 0) {
    console.log('URL da imagem:', `http://localhost:1337${imovel.imagens.data[0].attributes.url}`);
  }
      ...
      <div className="relative h-48 bg-gray-200">
        {imovel.imagens && imovel.imagens.length > 0 ? (
          <img
            ...
            onError={() => {
              console.log('Erro ao carregar imagem:', `http://localhost:1337${imovel.imagens[0].url}`);
              setImageError(true);
            }}
            onLoad={() => console.log('Imagem carregada:', imovel.titulo)}
          />
        ) : (
...
function App() {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      const requests = [
        ...
        // Buscar todos os imóveis primeiro, filtrar depois no frontend se necessário
      ...
      if (imoveisResponse.ok) {
        ...
        // Filtro por cliente no frontend
        if (tenant?.id) {
          const originalCount = properties.length;
          properties = properties.filter(imovel => {
            // Verificar se o imóvel tem cliente associado e se o ID corresponde
            const hasCorrectClient = imovel.cliente && 
                                   imovel.cliente.id && 
                                   imovel.cliente.id.toString() === tenant.id.toString();
            
            if (hasCorrectClient) {
              console.log(`✓ Imóvel "${imovel.titulo}" pertence ao cliente ${tenant.nome}`);
            }
            
            return hasCorrectClient;
          });
          
          console.log(`Filtro aplicado: ${originalCount} → ${properties.length} imóveis para cliente "${tenant.nome}" (ID: ${tenant.id})`);
        } else {
          console.log('Nenhum tenant selecionado, mostrando todos os imóveis');
        }
        
        if (properties.length > 0) {
          console.log('Primeiro imóvel filtrado:', JSON.stringify(properties[0], null, 2));
        }
  ...
  }, [API_BASE, tenant?.id, tenant?.nome]);
  ...
  const handleFilterChange = useCallback((filteredProperties) => {
    setImoveisFiltrados(filteredProperties);
  }, []);
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-gray-100">
        <div className="text-center">
          ...
          <div className="text-gray-500">Conectando com o servidor...</div>
        </div>
      </div>
    );
  }
